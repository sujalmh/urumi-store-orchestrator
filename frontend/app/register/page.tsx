...
export default function RegisterPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setError(apiError.detail || "Unable to create account");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-hero-glow p-4">
      <div className="w-full max-w-sm space-y-6">
        ...
        <p className="text-xs uppercase tracking-[0.2em] text-dusk/70">Start provisioning</p>
          <h1 className="mt-2 text-2xl font-bold tracking-tight text-ink">Create account</h1>
        </div>
        ...
        <p className="text-center text-sm text-dusk/60">
          ...
          <Link href="/login" className="font-medium text-ink hover:text-dusk transition-colors">
            Sign in
          </Link>
        </p>
      </div>
